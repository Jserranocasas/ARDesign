...
namespace GoogleARCore.Examples.ObjectManipulation
{
    ...
    public abstract class Manipulator : MonoBehaviour
    {
        ...
        public void Select()
        {
            ...
            if (ModeAction.Instance.getMode() != ModeStatus.Ruler &&
                ModeAction.Instance.getMode() != ModeStatus.Option)
            {
                ManipulationSystem.Instance.Select(gameObject);

                // Change current selected object
                Session.SetIdCurrentObject(gameObject.GetHashCode().ToString());
